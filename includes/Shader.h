...
namespace LearnOpenGLUtils
{
	void dealWithShaderCompilationError(GLuint shader)
	{
		...
		if (!success)
		{
			...
			throw LearnOpenGLUtils::ShaderCompilationException("ERROR::SHADER::VERTEX::COMPILATION_FAILED", infoLog);
		}
	}
	...
	void dealWithShaderLinkingError(GLuint shaderProgram)
	{
		...
		if (!success)
		{
			...
			std::cerr << infoLog << std::endl;
			throw LearnOpenGLUtils::ShaderLinkingException("ERROR::SHADER::PROGRAM::LINKING_FAILED", infoLog);
		}
	}
	...
	public:
		// 构造器读取并构建着色器
		Shader(const char *vertexPath, const char *fragmentPath, const char *prefix = nullptr);
		~Shader();
		// 使用/激活程序
		void use() const
		{
	...
	public:
		...
		~ShaderGuard()
		{
			glUseProgram(mOldShaderID);
		}
	};

	Shader::Shader(const char *vertexShaderSourcePath_, const char *fragmentShaderSourcePath_, const char *prefix)
	{
		...
		if (prefix != nullptr)
		{
			vertexShaderSourcePath = std::filesystem::path(prefix).append(vertexShaderSourcePath_);
			fragmentShaderSourcePath = std::filesystem::path(prefix).append(fragmentShaderSourcePath_);
		}
