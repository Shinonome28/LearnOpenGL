...
int TransformBasis() {
  ...
  LearnOpenGLUtils::Shader shader("transformBasis.vs.glsl",
                                  "transformBasis.fs.glsl", shaderPath.c_str());
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    // 参数解释如下：1. Uniform Location 2. 多少个矩阵 3. 是否需要转置 4.
    // 矩阵的值（使用GLM的工具函数返回适合于OpenGL的数据表示）
...
int main() { return TransformBasis(); }